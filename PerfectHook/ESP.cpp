...
void visuals::OnPaintTraverse(C_BaseEntity* local)
{
    ...
    for (int i = 0; i < g_EntityList->GetHighestEntityIndex(); i++)
    {
        ...
        if (entity && entity != local && !entity->IsDormant())
        {
            ...
            {
                if (g_Options.Legitbot.backtrackdot)
                {
                    if (local->IsAlive())
                    {
                        for (int t = 0; t < 12; ++t)
...
void visuals::DrawPlayer(C_BaseEntity* entity, player_info_t pinfo, C_BaseEntity* local)
{
	...
	{
		g_Render->Textf(int(top.x + width + 3), int(top.y + 12), Color(0, 255, 0, 255), g_Render->font.ESP, "%i$", entity->GetMoney());
	}
...
void visuals::DrawBombPlanted(C_BaseEntity* entity, C_BaseEntity* local)
{
	...
	if (g_Render->WorldToScreen(vOrig, vScreen))
	{
		if (local->IsAlive())
		{
			...
			sprintf_s(bombdamagestringdead, sizeof(bombdamagestringdead) - 1, "-100!");
	...
	if (Bomb->GetBombDefuser() > 0)
	{
		...
		if (countdown > 0.01f)
		{
			...
			{
				g_Render->Text(halfX - 50, halfY + 200, Color(255, 0, 0, 255), g_Render->font.Defuse, "-100!");
			}
		}
	}
}
